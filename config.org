...
** compilation
   #+begin_src emacs-lisp :tangle yes
	 ...
	 ;; C++
	 (defun compile-all-the-things (arg)
	   (interactive "p")
	   (if-let* ((root (vc-root-dir))
				 (cmake (concat root "/CMakeLists.txt"))
				 (build (concat root "/build")))
		   (call-process "cmake" nil nil nil "-S" root "-B" build "-GNinja" (when arg "-DCMAKE_CXX_COMPILER_LAUNCHER=/usr/lib/icecream/bin/icecc"))
...
** lsp
   #+begin_src emacs-lisp :tangle yes
	 (use-package lsp-mode
	   :hook
	   ((c++-mode c-mode rust-mode go-mode csharp-mode) . lsp)
	   :custom
...
* hotkeys
  #+begin_src emacs-lisp :tangle yes
	...
	(evil-leader/set-key
	  ...
	  "f p" '(lambda () (interactive) (ranger (expand-file-name "~/git/")))
	  "f u" '(lambda () (interactive) (ranger (expand-file-name "~/uni/")))
	  "f h" '(lambda () (interactive) (ranger (expand-file-name "~/")))
	  "f d" 'ranger
	  "f j" 'find-journal
	  ...
	  "+" 'calc
	  "-" 'mu4e
	  "<return>" '(lambda () (interactive) (split-window-sensibly) (other-window 1) (vterm-toggle-cd))
	...
	(evil-define-key nil ranger-mode-map
	  (kbd "d d") 'ranger-to-dired)
