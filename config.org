	 ...
	 (use-package exwm
	   ...
	   (exwm-input-global-keys
		`(
		  ...
		  ,@(mapcar (lambda (i) `(,(kbd (format "s-%d" (+ 1 i))) . (lambda () (interactive) (exwm-workspace-switch-create ,i))))
					(number-sequence 0 8))))
...
** helm
  ...
  (straight-use-package '(helm-fzf :type git :repo "https://github.com/ibmandura/helm-fzf"))
   #+end_src
* utils
** elfeed
   #+begin_src emacs-lisp :tangle yes
   ...
   #+begin_src emacs-lisp :tangle yes
	 ...
	 (lsp-register-client
	  (make-lsp-client :new-connection (lsp-tramp-connection "clangd")
					   ...
					   :multi-root t))
	 (lsp-register-client
	  (make-lsp-client :new-connection (lsp-tramp-connection "nimlsp")
...
** shell
   ...
   #+end_src
** COMMENT calctex
   Calctex is really cool for calc mode, although it's currently broken
   #+begin_src emacs-lisp :tangle yes
     (straight-use-package '(calctex :type git :repo "https://github.com/johnbcoughlin/calctex"))
     (add-hook 'calc-mode-hook 'calctex-mode)
   #+end_src
** avy
   Avy makes jumping to visible places very fast
   #+begin_src emacs-lisp :tangle yes
...
** which key
   ...
   #+end_src
** treemacs
   Treemacs makes emacs look like vscode... Not really, but it adds a filebrowser and some very nifty LSP integrations
   #+begin_src emacs-lisp :tangle yes
