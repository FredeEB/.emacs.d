   ...
   #+begin_src emacs-lisp :tangle yes
	 ...
	 (defun compile-c++-things ()
	   ...
	   (if-let* ((root (vc-root-dir))
		   ...
		   (progn (async-start-process
				   "Configure" "cmake"
				   `(lambda (arg)
					  (kill-buffer (process-buffer arg))
					  (call-process "ln" nil nil nil "-s" (concat ,build "/compile_commands.json") (concat ,root "/compile_commands.json"))
					  (helm-make-projectile (-> (process-lines "nproc") first string-to-number)))
				   "-S" root "-B" build "-GNinja" "-DCMAKE_EXPORT_COMPILE_COMMANDS=ON"))
